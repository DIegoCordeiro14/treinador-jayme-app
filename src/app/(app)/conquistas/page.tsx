...
import { EdnScoreCard } from '@/components/gamification/edn-score-card';
...
const ALL_ACHIEVEMENTS = [
  ...
  { id: 'progression_4w',   title: '4 Semanas de Progressão',description: 'Progrida por 4 semanas seguidas',      icon: '📈', xp: 150 },
  ...
  { id: 'ai_consultation',  title: 'Consultor Regular',      description: '10 consultas com o Treinador IA',       icon: '🤖', xp: 75  },
  { id: 'league_ouro',      title: 'Liga Ouro',              description: 'Atingiu Score EDN ≥ 60',               icon: '🥇', xp: 200 },
  { id: 'league_platina',   title: 'Liga Platina',           description: 'Atingiu Score EDN ≥ 75',               icon: '💎', xp: 400 },
  { id: 'league_elite',     title: 'Elite EDN',              description: 'Atingiu Score EDN ≥ 95',               icon: '⚡', xp: 1000},
];
...
export default function ConquistasPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return;
      const [{ data: xp }, { data: achievements }, { data: logs }, { count }] = await Promise.all([
  ...
  const sessionProgress: Record<string, { current: number; total: number }> = {
  ...
  // Build EDN breakdown from persisted scores or defaults
  const ednBreakdown = buildEdnBreakdown(
    userXP?.consistency_score ?? 0,
    userXP?.progression_score ?? 0,
    userXP?.nutrition_score   ?? 0,
    userXP?.cardio_score      ?? 0,
    userXP?.recovery_score    ?? 0,
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in-0 duration-300">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-0.5">Score EDN, XP e recompensas da sua jornada</p>
      </div>

      {/* EDN Score Card */}
      {!loading && <EdnScoreCard breakdown={ednBreakdown} />}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="font-bold text-zinc-100 text-sm">Nível {progress.level}</p>
      ...
      {/* Achievements grid */}
      <div>
        <h2 className="font-semibold text-zinc-100 mb-3">Conquistas ({earnedIds.size}/{ALL_ACHIEVEMENTS.length})</h2>
            ...
            return (
              <div key={def.id} className={cn('rounded-xl border p-4 transition-all', isEarned ? 'border-blue-600/30 bg-blue-600/5' : 'border-zinc-800 bg-zinc-900 opacity-50')}>
                ...
                {isEarned && earnedRecord && (
                  <p className="text-[10px] text-zinc-600 mt-2">{new Date(earnedRecord.earned_at).toLocaleDateString('pt-BR')}</p>
                )}
                {!isEarned && prog && (() => {
